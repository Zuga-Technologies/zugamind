...
Winner selection is weighted-random (salience^power over the global `random`
module), so the probabilistic tests below seed `random` for reproducibility —
a failure replays identically instead of vanishing on rerun.
"""
import random
...
def test_winner_is_broadcast_to_all_modules():
    ...
    b = _FixedBidModule("b", 0.01)  # 0.9^4 vs 0.01^4 — "a" wins under this seed
...
def test_registered_modulator_reweights_before_attention_schema():
    ...
    # Counterpart stays at 0.05 so the boosted bid dominates the draw.
