...
# --- rate limiter fails CLOSED when the journal is unreadable ---------------

def test_unreadable_journal_refuses_invocation_not_resets_count(tmp_path, monkeypatch):
    """The caps are counted from the journal. If the journal exists but can't
    be read, the count is unknowable — the actuator must refuse (fail closed),
    not treat the count as zero (which would silently erase both rate limits
    exactly when the audit trail is already broken)."""
...
def test_missing_journal_counts_as_zero_not_indeterminate(tmp_path, monkeypatch):
    """A fresh install genuinely has no history — a missing journal file is a
    legitimate zero, not an error."""
    _patch_journal(tmp_path, monkeypatch)  # file does not exist yet
    result = command_actuator.invoke_harness(_config(), "b", dry_run=True)
    assert result["ok"] is True
